...
recursive SUBROUTINE rgbi3p(md, nxd, nyd, xd, yd, zd, nip, xi, yi, zi, ier)
...
recursive SUBROUTINE rgsf3p(md, nxd, nyd, xd, yd, zd, nxi, xi, nyi, yi, zi, ier)
...
recursive FUNCTION z2f(xx1, xx2, zz0, zz1) RESULT(fn_val)

REAL(kind=dbl), INTENT(IN)  :: xx1, xx2, zz0, zz1
REAL(kind=dbl)              :: fn_val
...
recursive FUNCTION z3f(xx1, xx2, xx3, zz0, zz1, zz2) RESULT(fn_val)

REAL(kind=dbl), INTENT(IN)  :: xx1, xx2, xx3, zz0, zz1, zz2
REAL(kind=dbl)              :: fn_val
...
recursive SUBROUTINE rgpd3p(nxd, nyd, xd, yd, zd, pdd)
...
INTEGER(kind=irg) :: ipex, ipey, ix0, ix1, ix2, ix3, iy0, iy1, iy2, iy3, nx0, ny0
!     ..
!     .. Local Arrays ..
REAL(kind=dbl)    :: b00xa(4), b00ya(4), b01a(4), b10a(4), cxa(3,4), cya(3,4),   &
           sxa(4), sxxa(4), sya(4), syya(4), xa(3,4), ya(3,4),   &
           z0ia(3,4), zi0a(3,4)
! INTEGER(kind=irg) :: idlt(3,4)
!     ..
!     .. Intrinsic Functions ..
! INTRINSIC        MAX
!     ..
...
INTEGER(kind=irg), SAVE  :: idlt(3,4) = RESHAPE(   &
...
! Selects and/or supplements the x and z values.
      ...
      ELSE IF (nxd == 3) THEN
        x2 = xd(ix2) - x0
        z20 = zd(ix2,iy0)
        x3 = 2.D0*xd(3) - xd(2) - x0
        z30 = z3f(x1,x2,x3,z00,z10,z20)
      ELSE IF (nxd == 2) THEN
        x2 = 2.D0*xd(2) - xd(1) - x0
        z20 = z2f(x1,x2,z00,z10)
        x3 = 2.D0*xd(1) - xd(2) - x0
...
! Selects and/or supplements the y and z values.
      ...
      ELSE IF (nyd == 3) THEN
        y2 = yd(iy2) - y0
        z02 = zd(ix0,iy2)
        y3 = 2.D0*yd(3) - yd(2) - y0
        z03 = z3f(y1,y2,y3,z00,z01,z02)
      ELSE IF (nyd == 2) THEN
        y2 = 2.D0*yd(2) - yd(1) - y0
        z02 = z2f(y1,y2,z00,z01)
        y3 = 2.D0*yd(1) - yd(2) - y0
...
recursive SUBROUTINE rglctn(nxd, nyd, xd, yd, nip, xi, yi, inxi, inyi)
...
recursive SUBROUTINE rgplnl(nxd, nyd, xd, yd, zd, pdd, nip, xi, yi, inxi, inyi, zi)
...
INTEGER(kind=irg) :: iip, ixd0, ixd1, ixdi, ixdipv, iyd0, iyd1, iydi, iydipv
!     ..
!     .. Intrinsic Functions ..
! INTRINSIC        MAX
!     ..
...
! Calculates the polynomial coefficients.
      ...
      p00 = z00
      p01 = zy00
      p02 = (2.D0* (z0dy-zy00)+z0dy-zy01)/dy
      p03 = (-2.D0*z0dy+zy01+zy00)/dysq
      p10 = zx00
      p11 = zxy00
      p12 = (2.D0* (zx0dy-zxy00)+zx0dy-zxy01)/dy
      p13 = (-2.D0*zx0dy+zxy01+zxy00)/dysq
      p20 = (2.D0* (z0dx-zx00)+z0dx-zx10)/dx
      p21 = (2.D0* (zy0dx-zxy00)+zy0dx-zxy10)/dx
      p22 = (3.D0* (3.D0*a-b-c)+d)/ (dx*dy)
      p23 = (-6.D0*a+2.D0*b+3.D0*c-d)/ (dx*dysq)
      p30 = (-2.D0*z0dx+zx10+zx00)/dxsq
      p31 = (-2.D0*zy0dx+zxy10+zxy00)/dxsq
      p32 = (-6.D0*a+3.D0*b+2.D0*c-d)/ (dxsq*dy)
      p33 = (2.D0* (2.D0*a-b-c)+d)/ (dxsq*dysq)
    END IF
...
END MODULE mod_Grid_Interpolation
