...
!f2py intent(in,out) ::  out
    ...
    if(all(iOut.ne.nint(real(iIn) * s))) then ! make sure s * iIn == iOut
      call Message%printError('RescaleImage', 'could not find single scale factor to rescale both directions')
    endif
...
!f2py intent(in,out) ::  this ! structure to initialize
    ...
    if(s.le.0.D0) call Message%printError('ImageRescaler_Init', 'scale factor must be non-negative')
