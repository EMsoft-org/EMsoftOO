c
c this f77 routine and its dependencies were downloaded from netlib.org on 07/24/23 by MDG
c and included in the EMsoftOOLib folder
c
c link: https://www.netlib.org/cgi-bin/netlibfiles.pl?filename=/minpack/lmdif.f
c
c they are part of the original minpack package developed at Argonne National Laboratory
c in the 1980s
c
c
c this routine is used in the EMHREBSD program to fit a 2D Gaussian to a cross-correlation peak
c to recover the peak position with sub-pixel accuracy
c
c
c Copyright disclaimer from netlib.org:
